...
-- Update existing data to assign to current user (for demo purposes, this would normally be done differently)
-- This is just for existing data - new data will have user_id set automatically
UPDATE public.raw_materials SET user_id = auth.uid() WHERE user_id IS NULL;
...
UPDATE public.suppliers SET user_id = auth.uid() WHERE user_id IS NULL;
UPDATE public.sales_records SET user_id = auth.uid() WHERE user_id IS NULL;
UPDATE public.production_records SET user_id = auth.uid() WHERE user_id IS NULL;
UPDATE public.product_bom SET user_id = auth.uid() WHERE user_id IS NULL;
UPDATE public.stock_movements SET user_id = auth.uid() WHERE user_id IS NULL;
